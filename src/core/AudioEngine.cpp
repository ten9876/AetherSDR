    ...
    : QObject(parent)
{
    ...
    connect(m_rxTimer, &QTimer::timeout, this, [this]() {
        ...
        // Local PC sidetone (#1075): when key is held and local sidetone is
        // enabled, generate a sine tone and mix it into whatever free space
        // the sink has, so the operator hears their own keying locally.
        if (m_localSidetoneEnabled.load() && m_localSidetoneKeyDown.load()) {
            qsizetype free = m_audioSink->bytesFree();
            if (free > 0) {
                // Generate one timer-tick worth of stereo int16 samples.
                // We cap to 20ms so we never flood the sink.
                const int rate = m_resampleTo48k ? 48000 : DEFAULT_SAMPLE_RATE;
                const int maxSamples = rate / 50;  // 20ms worth of stereo pairs
                const int stereoSamples = static_cast<int>(
                    std::min(static_cast<qsizetype>(maxSamples),
                             free / static_cast<qsizetype>(4)));  // 4 bytes per stereo pair
                const double pitch = static_cast<double>(m_sidetonePitch.load());
                const float  gain  = m_sidetoneGain.load();
                QByteArray tone(stereoSamples * 4, Qt::Uninitialized);
                auto* out = reinterpret_cast<int16_t*>(tone.data());
                const double phaseInc = 2.0 * M_PI * pitch / static_cast<double>(rate);
                for (int i = 0; i < stereoSamples; ++i) {
                    const auto s = static_cast<int16_t>(
                        std::sin(m_sidetonePhase) * gain * 32767.0f);
                    out[i * 2]     = s;   // L
                    out[i * 2 + 1] = s;   // R
                    m_sidetonePhase += phaseInc;
                    if (m_sidetonePhase >= 2.0 * M_PI) m_sidetonePhase -= 2.0 * M_PI;
                }
                m_audioDevice->write(tone);
            }
        } else {
            // Reset phase so next key-down starts cleanly (no click).
            m_sidetonePhase = 0.0;
        }
...
// Local PC sidetone key state (#1075).
// Thread-safe: only writes the atomic flag; the audio-thread timer handles phase.
void AudioEngine::setLocalSidetoneKeyDown(bool down)
{
    m_localSidetoneKeyDown.store(down);
}
