...
void VirtualAudioBridge::feedDaxAudio(int channel, const QByteArray& pcm)
{
    ...
    // Debug: log first few calls per channel
    static int feedDebugCount[5] = {};
    if (feedDebugCount[channel]++ < 5) {
        qDebug() << "VirtualAudioBridge::feedDaxAudio ch=" << channel
                 << "pcm.size=" << pcm.size()
                 << "shm active=" << block->active
                 << "writePos=" << block->writePos.load();
    }
