...
void SpectrumWidget::mousePressEvent(QMouseEvent* ev)
{
    ...
    if (y >= scaleY && y < scaleY + FREQ_SCALE_H) {
        ...
        m_bwDragStartCenter = m_centerMhz;  // snapshot center so zoom stays anchored (#1093)
...
void SpectrumWidget::mouseMoveEvent(QMouseEvent* ev)
{
    ...
    if (m_draggingBandwidth) {
        ...
        // Other modes: keep the pan center fixed at drag-start position so the
        // VFO does not snap to the center of the display (#1093).
        double zoomCenter = m_bwDragStartCenter;
