...
#include "core/plugins/IAmpPlugin.h"
...
RadioSetupDialog::RadioSetupDialog(RadioModel* model, AudioEngine* audio,
                                   ...
                                   AntennaGeniusModel* ag, PluginManager* plugins,
                                   QWidget* parent)
    : QDialog(parent), m_model(model), m_audio(audio),
      m_tgxl(tgxl), m_pgxl(pgxl), m_ag(ag), m_plugins(plugins)
{
...
QWidget* RadioSetupDialog::buildPeripheralsTab()
{
    ...
    // ── Amplifier Plugins section (#1109) ────────────────────────────────────
    if (m_plugins) {
        auto* plugGroup = new QGroupBox("Amplifier Plugins");
        plugGroup->setStyleSheet(kGroupStyle);
        auto* plugLayout = new QVBoxLayout(plugGroup);
        plugLayout->setSpacing(4);

        const auto& plugins = m_plugins->ampPlugins();
        if (plugins.isEmpty()) {
            auto* emptyLbl = new QLabel(
                "No amp plugins loaded.\n"
                "Place .so / .dll files implementing IAmpPlugin into:\n"
                "  ~/.config/AetherSDR/plugins/amp/");
            emptyLbl->setStyleSheet("QLabel { color: #607080; font-size: 11px; }");
            plugLayout->addWidget(emptyLbl);
        } else {
            // Column headers
            ...
            auto makeHdr = [](const QString& t) {
                auto* l = new QLabel(t);
                l->setStyleSheet("QLabel { color: #8aa8c0; font-size: 11px; font-weight: bold; }");
                return l;
            };
            headerLayout->addWidget(makeHdr("Plugin"), 3);
            headerLayout->addWidget(makeHdr("Version"), 1);
            headerLayout->addWidget(makeHdr("Amp Model"), 4);
            plugLayout->addWidget(headerRow);
            ...
            for (auto* plugin : plugins) {
                ...
                auto* nameLbl = new QLabel(plugin->pluginName());
                nameLbl->setStyleSheet(kValueStyle);
                auto* verLbl  = new QLabel(plugin->pluginVersion());
                verLbl->setStyleSheet(kLabelStyle);
                auto* modelLbl = new QLabel(plugin->ampModel());
                modelLbl->setStyleSheet(kLabelStyle);
                auto* statusLbl = new QLabel("Loaded");
                statusLbl->setStyleSheet("QLabel { color: #00e060; font-size: 11px; }");

                rowLayout->addWidget(nameLbl, 3);
                rowLayout->addWidget(verLbl, 1);
                rowLayout->addWidget(modelLbl, 4);
                rowLayout->addWidget(statusLbl, 1);
                plugLayout->addWidget(row);
            }
        }

        auto* plugNote = new QLabel(
            "Amp plugins are loaded from ~/.config/AetherSDR/plugins/amp/ at startup.\n"
            "Each plugin appears as a tab in the AMP applet panel.");
        plugNote->setWordWrap(true);
        plugNote->setStyleSheet("QLabel { color: #607080; font-size: 11px; padding-top: 6px; }");
        plugLayout->addWidget(plugNote);

        vbox->addWidget(plugGroup);
    }
