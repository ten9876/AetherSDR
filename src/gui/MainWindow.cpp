...
void MainWindow::wirePanadapter(PanadapterApplet* applet)
{
    ...
    connect(menu, &SpectrumOverlayMenu::bandSelected,
        ...
        if (stackKey.isEmpty()) {
            ...
            // Re-assert audio_mute=0 for unmuted slices on this pan after the
            // radio processes the band change.  Flex firmware may silently drop
            // audio routing during band transitions in multi-slice setups
            // without sending an audio_mute status echo (#2064).
            const QString panId = applet->panId();
            QTimer::singleShot(300, this, [this, panId]() {
                for (auto* s : m_radioModel.slices()) {
                    if (s->panId() == panId && !s->audioMute()) {
                        m_radioModel.sendCommand(
                            QString("slice set %1 audio_mute=0").arg(s->sliceId()));
                    }
                }
            });
        }
