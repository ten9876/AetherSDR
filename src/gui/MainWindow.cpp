    ...
    : QMainWindow(parent)
{
    ...
    // Local PC sidetone toggle/volume (#1075): forward to AudioEngine.
    connect(m_appletPanel->phoneCwApplet(), &PhoneCwApplet::localSidetoneChanged,
            this, [this](bool enabled, int volume) {
        m_audio->setLocalSidetoneEnabled(enabled);
        m_audio->setLocalSidetoneGain(volume / 100.0f);
        AppSettings::instance().save();
    });
    // Apply persisted local sidetone settings on startup.
    {
        auto& s = AppSettings::instance();
        m_audio->setLocalSidetoneEnabled(s.value("LocalSidetoneEnabled", false).toBool());
        m_audio->setLocalSidetoneGain(s.value("LocalSidetoneVolume", 50).toInt() / 100.0f);
        m_audio->setLocalSidetonePitch(m_radioModel.transmitModel().cwPitch());
    }
    // Keep local sidetone pitch in sync with the radio's CW pitch (#1075).
    ...
    connect(m_serialPort, &SerialPortController::cwKeyChanged,
        ...
        // Drive local PC sidetone for straight key (#1075).
        // Pitch is synced from the radio's CW pitch setting.
        m_audio->setLocalSidetonePitch(m_radioModel.transmitModel().cwPitch());
        m_audio->setLocalSidetoneKeyDown(down);
    });
