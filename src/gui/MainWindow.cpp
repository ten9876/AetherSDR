    ...
    : QMainWindow(parent)
{
    ...
    connect(m_hidEncoder, &HidEncoderManager::buttonPressed,
            this, [this](int button, int action) {
        // Same action dispatch as FlexControl; button 1 defaults to CycleStep (#997)
        QString key = QString("HidEncoderBtn%1Action%2").arg(button).arg(action);
        static const char* defaults[2][2] = {
            {"CycleStep", "None"},
            {"None",      "None"},
        };
        const char* def = (button >= 1 && button <= 2 && action >= 0 && action <= 1)
                          ? defaults[button-1][action] : "None";
        QString actionName = AppSettings::instance().value(key, def).toString();

        if (actionName == "CycleStep") {
            if (auto* rx = m_appletPanel->rxApplet()) rx->cycleStepUp();
    ...
    connect(m_hidEncoder, &HidEncoderManager::connectionChanged,
        ...
        // Update encoder badge on all active VfoWidgets (#997)
        const auto& slices = m_radioModel.slices();
        for (auto* s : slices) {
            if (auto* sw = spectrumForSlice(s))
                if (auto* vfo = sw->vfoWidget(s->sliceId()))
                    vfo->setEncoderActive(connected);
        }
    });
